...
void welcomeView() {//처음시작화면함수
	...
	center("Press to start", 7);//가운데에 출력
	center("Select 2 or 3", 9);
	center("", WH);
...
int main() {
	...
	while (1) {
		display(); //화면 출력함수 호출
		if(multiple==50) addRandomValue(doProcess(getAction())); //셀에 임의의 숫자 대입
		if(multiple==51) addRandomValue1(doProcess(getAction()));
	}
	return 0;
}
